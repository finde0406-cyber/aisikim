...
const BRAND_CONFIGS: Record<BrandId, BrandConfig> = {
  ...
  'moneyflow-radar': {
    ...
    ctaVariants: [
      '머니플로우레이더처럼 시장 자금의 흐름을 같이 보면 리스크를 더 빨리 읽을 수 있습니다.',
      '시장을 볼 때 가격만 보지 말고 머니플로우레이더처럼 자금 흐름도 함께 체크해보세요.',
      '불안한 장세일수록 머니플로우레이더처럼 어디로 돈이 움직이는지 먼저 보는 편이 도움이 됩니다.',
    ],
    topics: [
      { id: 1, label: '시장 자금 흐름 점검', theme: '시장 자금 흐름 점검', hook: '지수가 버티는데도 왜 불안한지 설명이 안 되신 적 있으신가요?', before: '지수와 가격만 보고 방향 판단', after: '시장 안에서 돈이 어디로 이동하는지 함께 확인', saving: '표면 아래 위험 신호를 더 빨리 인지', quickTip: '가격이 버틴다고 안심하기보다 자금이 어떤 섹터로 이동하는지 같이 봐야 흐름이 보입니다.' },
      { id: 2, label: '리스크 오프 신호', theme: '리스크 오프 신호', hook: '시장이 멀쩡해 보이는데 자금은 이미 방어적으로 움직이고 있을 수 있습니다.', before: '뉴스 헤드라인만 보고 안심', after: '방어 자산과 민감 자산 간 자금 이동 비교', saving: '리스크 오프 전환을 더 빨리 감지', quickTip: '가격보다 자금 이동이 먼저 바뀌는 구간이 있어서 위험회피 신호는 흐름에서 먼저 보일 때가 많습니다.' },
      { id: 3, label: '섹터 로테이션 해석', theme: '섹터 로테이션 해석', hook: '같은 상승장이어도 어떤 섹터에 돈이 몰리는지는 완전히 다를 수 있습니다.', before: '상승/하락만 단순 해석', after: '섹터 간 자금 순환을 함께 점검', saving: '장세의 성격을 더 정확히 이해', quickTip: '섹터 로테이션을 보면 시장이 공격적으로 가는지 방어적으로 가는지 훨씬 선명해집니다.' },
      { id: 4, label: '유동성 방향 점검', theme: '유동성 방향 점검', hook: '시장이 오르는 이유가 실적 때문인지 유동성 때문인지 구분하고 계신가요?', before: '가격 상승 자체를 긍정 신호로 해석', after: '유동성 유입과 이탈 흐름을 같이 추적', saving: '상승의 질을 더 냉정하게 판단', quickTip: '유동성이 밀어 올리는 장은 빠르게 바뀔 수 있어서 돈의 속도와 방향을 같이 보는 게 중요합니다.' },
      { id: 5, label: '시장 내부 온도차', theme: '시장 내부 온도차', hook: '지수는 괜찮은데 체감은 불안한 날이 왜 생기는지 느껴보신 적 있으신가요?', before: '대표 지수 하나로 시장 전체를 판단', after: '시장 내부의 온도차와 자금 분산 흐름 확인', saving: '체감과 지수의 괴리를 더 잘 설명', quickTip: '지수는 버텨도 내부 종목과 섹터 흐름이 약해지면 체감 불안은 훨씬 먼저 커질 수 있습니다.' },
      { id: 6, label: '리스크 관리 타이밍', theme: '리스크 관리 타이밍', hook: '리스크 관리는 급락 후가 아니라 흐름이 바뀔 때 시작되는 경우가 많습니다.', before: '변동성 확대 후에야 대응 시작', after: '자금 이탈 조짐이 보일 때 먼저 점검', saving: '늦은 대응 가능성 축소', quickTip: '리스크 관리는 이벤트 발생 후보다 자금이 방향을 바꾸는 초기에 더 효과적일 때가 많습니다.' },
      { id: 7, label: '방어 자산 이동', theme: '방어 자산 이동', hook: '위험자산은 버티는데 안전자산으로 돈이 몰리면 그 신호를 보고 계신가요?', before: '자산별로 따로 보고 연결하지 않음', after: '방어 자산 유입을 시장 경고 신호로 점검', saving: '보이지 않는 불안감 조기 감지', quickTip: '안전자산으로 흐르는 돈은 시장 참가자들의 심리를 먼저 보여주는 경우가 많습니다.' },
      { id: 8, label: '시장 심리와 자금', theme: '시장 심리와 자금', hook: '시장 심리는 뉴스보다 돈의 방향에서 더 솔직하게 드러날 때가 많습니다.', before: '해석 위주의 뉴스 소비', after: '실제 자금 흐름으로 심리 변화를 추적', saving: '소음보다 본질에 집중', quickTip: '심리는 말보다 움직인 돈에서 더 잘 보이기 때문에 해석보다 흐름이 먼저일 때가 많습니다.' },
      { id: 9, label: '변동성 확대 전조', theme: '변동성 확대 전조', hook: '큰 변동성은 갑자기 시작되는 것 같지만, 흐름은 그 전에 흔들리기도 합니다.', before: '급락이 나와야 위험을 인식', after: '자금 분산과 이탈 패턴을 먼저 관찰', saving: '변동성 전조를 미리 체크', quickTip: '변동성 확대 전에는 시장 내부에서 자금이 먼저 갈라지거나 방어적으로 움직이는 경우가 잦습니다.' },
      { id: 10, label: '시장 구조 읽기', theme: '시장 구조 읽기', hook: '시장을 볼 때 가격만이 아니라 구조를 함께 읽고 계신가요?', before: '상승/하락 결과만 확인', after: '자금 흐름 기반으로 장의 구조를 해석', saving: '판단 기준이 더 구조화됨', quickTip: '시장 구조를 읽는다는 건 어디로 돈이 몰리고 어디서 빠지는지 지속적으로 보는 것에 가깝습니다.' },
    ],
  },
  'risk-check': {
    id: 'risk-check',
